"""
Embedding service — semantic text similarity using Google gemini-embedding-001.

Migrated to the google-genai SDK (google.genai).
The model produces 3072-dimensional dense embeddings.
Similarity scores >= 0.70 are reliable matches; 0.50-0.70 are weak; < 0.50 are misses.
"""
...
async def embed_text(text: str) -> list[float]:
    ...
    Result is cached for `embedding_cache_ttl` seconds.
    The SDK call is synchronous — offloaded to thread pool.
    """
    now = time.time()
...
def embed_sop_text(
    ...
    """Build a canonical embedding string from SOP metadata + first 3 steps."""
    parts: list[str] = [name]
    if description:
...
async def find_best_sop(
    ...
    Returns (sop_id, score) of the best match, or None if below threshold.
    """
    if not candidates:
        return None
    ...
    for cand in candidates:
        ...
        if not stored_emb:
            sop_text = embed_sop_text(cand.get("name", ""), cand.get("description"))
    ...
    if best_id:
        logger.info("Semantic SOP match: sop_id=%s score=%.3f query=%r",
                    best_id, best_score, query[:60])
    else:
        logger.info("No SOP match above threshold %.2f for query=%r",
